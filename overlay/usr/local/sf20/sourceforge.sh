#!/usr/bin/env bash
# sourceforge.sh
# SourceForge 2.0 helper: Gitea server/runner + MySQL/MariaDB + No-IP (DDNS) + HTTPS (Nginx+Let's Encrypt) on Pi/Debian.
set -euo pipefail
...
supports_utf8() { locale charmap 2>/dev/null | grep -qi 'utf-8'; }

# ASCII defaults
C_RESET=$'\e[0m'; C_BOLD=$'\e[1m'; C_DIM=$'\e[2m'
C_PRIM_BG=''; C_PRIM_TX=''; C_HEAD_TX=''
C_ACC=$'\e[36m'; C_MUTED=$'\e[90m'; C_OK=$'\e[32m'
C_WARN=$'\e[33m'; C_ERR=$'\e[31m'; C_LINK=$'\e[36m'; C_BOX=$'\e[34m'

# Unicode line art (fallbacks to ASCII if not supported)
CHR_H='-' ; CHR_V='|' ; CHR_TL='+' ; CHR_TR='+' ; CHR_BL='+' ; CHR_BR='+' ; CHR_SEP='-'
if supports_utf8; then
  CHR_H='?' ; CHR_V='?' ; CHR_TL='?' ; CHR_TR='?' ; CHR_BL='?' ; CHR_BR='?' ; CHR_SEP='?'
fi

# Color upgrade if 256-color
...
hr(){ local w; w=$(term_width); printf '%s\n' "$(printf '%*s' "$w" '' | tr ' ' "$CHR_SEP")"; }
pad_center(){ local t="$1" w; w=$(term_width); local l=${#t}; local p=$(( (w-l)/2 )); ((p<0))&&p=0; printf '%*s%s%*s\n' "$p" '' "$t" "$p" ''; }

banner(){ # title + subtitle
  local title="$1" sub="${2-}" w; w=$(term_width)
  if [[ -n "$C_PRIM_BG" ]]; then
    printf "${C_PRIM_BG}${C_HEAD_TX}${C_BOLD}%s\n" "$(printf '%*s' "$w" ' ' )"
    pad_center "?  SourceForge 2.0"
    pad_center "$title"
    [[ -n "$sub" ]] && pad_center "${C_PRIM_TX}${sub}${C_HEAD_TX}"
    printf "%s${C_RESET}\n" "$(printf '%*s' "$w" ' ' )"
  else
    hr
    pad_center "SourceForge 2.0 ? $title"
    [[ -n "$sub" ]] && pad_center "$sub"
    hr
  fi
}
...
section(){ local t="$1"
  ...
  printf "${C_BOX}%s${C_RESET}\n" "${CHR_TL}$(printf '%*s' $((w-2)) '' | tr ' ' "$CHR_H")${CHR_TR}"
  printf "${C_BOX}%s${C_RESET} ${C_BOLD}${C_ACC}%s${C_RESET}\n" "$CHR_V" "$t"
  printf "${C_BOX}%s${C_RESET}\n" "${CHR_BL}$(printf '%*s' $((w-2)) '' | tr ' ' "$CHR_H")${CHR_BR}"
}

msg_ok(){ printf "${C_OK}? %s${C_RESET}\n" "$*"; }
msg_info(){ printf "${C_ACC}? %s${C_RESET}\n" "$*"; }
...
ops_tools_menu(){
  while true; do
    ...
    echo "${C_BOX}${CHR_TL}$(printf '%*s' $(( $(term_width)-2 )) '' | tr ' ' "$CHR_H")${CHR_TR}${C_RESET}"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}1)${C_RESET} Reconfigure runner (purge & re-register)"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}2)${C_RESET} Update runner binary (external installer)"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}3)${C_RESET} Show runner service status"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}4)${C_RESET} Tail runner logs (last 200)"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}5)${CRESET} Network checks to Gitea (curl + nc)"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}6)${CRESET} Runner ? MySQL connectivity test (service user)"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}b)${C_RESET} Back"
    echo "${C_BOX}${CHR_BL}$(printf '%*s' $(( $(term_width)-2 )) '' | tr ' ' "$CHR_H")${CHR_BR}${C_RESET}"
...
dns_noip_menu(){
  while true; do
    ...
    echo "${C_BOX}${CHR_TL}$(printf '%*s' $(( $(term_width)-2 )) '' | tr ' ' "$CHR_H")${CHR_TR}${C_RESET}"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}1)${C_RESET} Install/Configure No-IP (ddclient)"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}2)${CRESET} Force update now"
    echo "${C_BOX}${CHR_V}${CRESET} ${C_BOLD}3)${CRESET} Show ddclient service status"
    echo "${C_BOX}${CHR_V}${CRESET} ${C_BOLD}4)${CRESET} Uninstall No-IP (ddclient)"
    echo "${C_BOX}${CHR_V}${CRESET} ${C_BOLD}b)${CRESET} Back"
    echo "${C_BOX}${CHR_BL}$(printf '%*s' $(( $(term_width)-2 )) '' | tr ' ' "$CHR_H")${CHR_BR}${C_RESET}"
...
mysql_menu(){
  while true; do
    ...
    echo "${C_BOX}${CHR_TL}$(printf '%*s' $(( $(term_width)-2 )) '' | tr ' ' "$CHR_H")${CHR_TR}${C_RESET}"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}1)${C_RESET} Server (install, create DB/user, integrate with Gitea)"
    echo "${C_BOX}${CHR_V}${CRESET} ${C_BOLD}2)${CRESET} Client (install and write client-app.ini)"
    echo "${C_BOX}${CHR_V}${CRESET} ${C_BOLD}3)${CRESET} Uninstall (server/client and data)"
    echo "${C_BOX}${CHR_V}${CRESET} ${C_BOLD}4)${CRESET} Test Server connection"
    echo "${C_BOX}${CHR_V}${CRESET} ${C_BOLD}5)${CRESET} Test Client connection"
    echo "${C_BOX}${CHR_V}${CRESET} ${C_BOLD}b)${CRESET} Back"
    echo "${C_BOX}${CHR_BL}$(printf '%*s' $(( $(term_width)-2 )) '' | tr ' ' "$CHR_H")${CHR_BR}${C_RESET}"
...
https_proxy_menu(){
  while true; do
    ...
    echo "${C_BOX}${CHR_TL}$(printf '%*s' $(( $(term_width)-2 )) '' | tr ' ' "$CHR_H")${CHR_TR}${C_RESET}"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}1)${C_RESET} Install & configure Nginx proxy for Gitea + obtain TLS cert"
    echo "${C_BOX}${CHR_V}${CRESET} ${C_BOLD}2)${CRESET} Force cert renewal (dry-run)"
    echo "${C_BOX}${CHR_V}${CRESET} ${C_BOLD}3)${CRESET} Show Nginx status / test config"
    echo "${C_BOX}${CHR_V}${CRESET} ${C_BOLD}4)${CRESET} Uninstall proxy config (keeps Gitea)"
    echo "${C_BOX}${CHR_V}${CRESET} ${C_BOLD}b)${CRESET} Back"
    echo "${C_BOX}${CHR_BL}$(printf '%*s' $(( $(term_width)-2 )) '' | tr ' ' "$CHR_H")${CHR_BR}${C_RESET}"
...
main_menu(){
  while true; do
    ...
    echo "${C_BOX}${CHR_TL}$(printf '%*s' $(( $(term_width)-2 )) '' | tr ' ' "$CHR_H")${CHR_TR}${C_RESET}"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}0)${C_RESET} Install Gitea server & configs"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}1)${C_RESET} Install runner"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}2)${C_RESET} Runner hook to Gitea (re-register)"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}3)${CRESET} Show smoke-test workflow snippet"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}4)${CRESET} Ops tools (reconfigure/update/logs/net + runner?MySQL test)"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}5)${CRESET} DNS (No-IP via ddclient)"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}6)${CRESET} Uninstall Gitea server"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}7)${CRESET} Uninstall runner"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}8)${CRESET} MySQL/MariaDB setup (Server/Client/Uninstall/Tests)"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}9)${CRESET} HTTPS reverse proxy (Nginx + Let's Encrypt)"
    echo "${C_BOX}${CHR_V}${C_RESET} ${C_BOLD}q)${CRESET} Quit"
    echo "${C_BOX}${CHR_BL}$(printf '%*s' $(( $(term_width)-2 )) '' | tr ' ' "$CHR_H")${CHR_BR}${C_RESET}"
