#!/usr/bin/env bash
# SourceForge 2.0 Infra Menu (Gitea/Runner/MySQL/DDNS/HTTPS)
set -euo pipefail
  ...
  while true; do
    ...
    case "${MODE:-}" in
      1)
        ...
        read -rsp "$(printf "${C_ACC}?${CRESET} Gitea DB user password: ")" DB_PASS; echo
...
https_proxy_menu(){
  while true; do
    ...
    echo "${C_BOX}┌──────────────────────────────────────────────────────────────────────────┐${C_RESET}"
    echo "${C_BOX}│${CRESET} ${C_BOLD}1)${C_RESET} Install & configure Nginx proxy for Gitea + obtain TLS cert                "
    echo "${C_BOX}│${CRESET} ${C_BOLD}2)${C_RESET} Force cert renewal (dry-run)                                                "
    echo "${C_BOX}│${CRESET} ${C_BOLD}3)${CRESET} Show Nginx status / test config                                            "
    echo "${C_BOX}│${CRESET} ${C_BOLD}4)${CRESET} Uninstall proxy config (keeps Gitea)                                         "
    echo "${C_BOX}│${CRESET} ${C_BOLD}b)${CRESET} Back                                                                  "
    echo "${C_BOX}└──────────────────────────────────────────────────────────────────────────┘${C_RESET}"
...
main_menu(){
  while true; do
    ...
    echo "${C_BOX}│${CRESET} ${C_BOLD}5)${C_RESET} DNS (No-IP via ddclient)                                                       "
    echo "${C_BOX}│${C_RESET} ${C_BOLD}6)${CRESET} Uninstall Gitea server                                                        "
    ...
    echo "${C_BOX}│${C_RESET} ${C_BOLD}9)${CRESET} HTTPS reverse proxy (Nginx + Let's Encrypt)                                   "
